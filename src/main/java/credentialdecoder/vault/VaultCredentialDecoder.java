...
public class VaultCredentialDecoder implements CredentialDecoder {
    ...
    public VaultCredentialDecoder(HttpClient httpClient) {
    ...
    final Function<HttpUriRequest, JSONObject> send = request -> {
        try {
            return client
                    .execute(
                            request,
    ...
    final Function<JSONObject, HttpUriRequest> tokenRequest = json -> {
    ...
    final Function<JSONObject, String> getToken =
            extractToken
                    .compose(send)
                    .compose(tokenRequest);

    final Function<JSONObject, JSONObject> getCreds =
            getjson
                    .apply("data")
                    .compose(send)
